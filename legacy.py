...
from PySide6.QtGui import QIcon, QFont, QColor, QAction, QPainter, QIntValidator, QTextOption
...
class CharacterFilterModule(PreprocessingModule):
    def __init__(self, chars_to_remove, char_match_type="Characters"):
        self.char_match_type = char_match_type
        if self.char_match_type == "Characters":
            escaped_chars = [re.escape(char) for char in chars_to_remove]
            self.pattern = re.compile(f"[{''.join(escaped_chars)}]")
        elif self.char_match_type == "Sequences":
            escaped_chars = [re.escape(char) for char in chars_to_remove]
...
class TokenizationModule(PreprocessingModule):
    def process(self, text):
...
class SpacingNormalizationModule(PreprocessingModule):
    def process(self, text):
...
class DocumentProcessor:
    ...
    def update_pipeline(self):
        ...
        if self.parameters["chars_to_remove"]:
            self.pipeline.add_module(CharacterFilterModule(self.parameters["chars_to_remove"], self.parameters["char_match_type"]))
        ...
        if self.parameters["use_tokenization"]:
            self.pipeline.add_module(TokenizationModule())
        ...
        if self.parameters["normalize_spacing"]:
            self.pipeline.add_module(SpacingNormalizationModule())
...
class ProcessingWorker(QRunnable):
    ...
    def run(self):
        try:
            ...
            processed_text, error = self.processor.process_file(self.file_path)
            end_time = time.time()
            processing_time = end_time - start_time
            if error:
                self.signals.warning.emit(self.file_path, error)
            elif processed_text:
...
class CharacterSelectionDialog(QDialog):
    def __init__(self, current_chars, parent=None):
        ...
        self.char_input.setPlaceholderText("Enter characters to remove")
        ...
        layout.addWidget(QLabel("Characters to remove:"))
    ...
    def add_chars(self):
        new_chars = self.char_input.text().strip()
        if new_chars and new_chars not in self.selected_chars:
            self.selected_chars.append(new_chars)
            self.update_char_list()
        self.char_input.clear()

    def update_char_list(self):
        self.char_list.clear()
        for chars in self.selected_chars:
            item = QListWidgetItem(chars)
            if len(chars) == 1:
    ...
    def get_selected_chars(self):
        return self.selected_chars, "Characters" if len(self.char_input.text().strip()) == 1 else "Sequences"
...
class ParametersDialog(QDialog):
    def __init__(self, current_parameters, parent=None):
        ...
        options = [
            ...
            ("use_tokenization", "Use Tokenization"),
        ...
        self.selected_chars_label = QLabel("Selected characters: " + ', '.join(self.parameters.get("chars_to_remove", [])))
    ...
    def open_regex_dialog(self):
        ...
        if self.pattern_data:
            for row_data in self.pattern_data:
                ...
                dialog.pattern_table.setCellWidget(row_position, 3).setText(row_data["number_length"])
        if dialog.exec():
    ...
    def open_char_selection(self):
        ...
        if dialog.exec():
            self.parameters["chars_to_remove"], self.parameters["char_match_type"] = dialog.get_selected_chars()
            self.selected_chars_label.setText("Selected characters: " + ', '.join(self.parameters["chars_to_remove"]))
...
class PreprocessorGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.signals.processing_complete.connect(self.display_results)
    ...
    def display_results(self, results, warnings):
        if results:
            ...
            for file_path, processed_text in results:
                ...
                if self.processor.parameters["use_tokenization"]:
        ...
        self.update_report(processed=True, processed_results=results)
    ...
    def show_about_dialog(self):
        QMessageBox.about(self, "About", "CorpuScript\nVersion 0.2\n\nDeveloped by Jhonatan Lopes")
